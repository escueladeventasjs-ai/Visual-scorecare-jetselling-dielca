...
function init(){buildInputs();buildModeratorButtons();load();selectModeratorTeam('A');render();}
...
function selectModeratorTeam(id){
  ...
  box.innerHTML=`<h3>${TEAMS[id][0]} · ${TEAMS[id][1]} · ${phase==='prospection'?'Prospección':'Negociación'}</h3><p class="hint">Mismos criterios que usa el jurado en esta fase.</p><div class="criteria">`+
    labels.map((c,i)=>`<div class="criterion"><label>${c[0]}</label><small>Peso: ${c[1]}%</small><input id="${key(id,phase,i)}" type="range" min="0" max="10" value="${getSaved(key(id,phase,i),5)}" data-weight="${c[1]}" oninput="saveAndRender(); updateModeratorTotal('${id}')"><b id="v_${key(id,phase,i)}">${getSaved(key(id,phase,i),5)}</b></div>`).join('')+
    `</div><div class="mod-total" id="modTotal">Moderador: 50/100</div>`;
  updateModeratorTotal(id);
}
function getSaved(k,fallback){
  const raw=localStorage.getItem('dielca-score-mod60-brand-embedded');
...
function moderatorScore(id){return Math.round((moderatorPhaseScore(id,'prospection')+moderatorPhaseScore(id,'negotiation'))/2)}
...
function finalScore(id){const jury=juryAverage(id),mod=moderatorScore(id);return Math.round((jury*.4+mod*.6)*10)/10}
...
function render(){
  ...
  document.getElementById('ranking').innerHTML=data.map((x,i)=>{
    ...
    return `<div class="rank ${i===0?'first':''}"><div class="medal">${medals[i]}</div><div><div class="name">${x.name}</div><div class="sub">${x.sub} · Jurado ${x.jury}/100 · Moderador ${x.mod}/100</div></div><div class="pts">${x.final}</div><div class="bar"><div style="width:${width}%"></div></div></div>`
  }).join('');
  ...
  document.getElementById('publicAward').innerHTML=publicData.map((x,i)=>{
    ...
    return `<div class="rank ${i===0?'first':''}"><div class="medal">${medals[i]}</div><div><div class="name">${x.name}</div><div class="sub">${x.sub} · Prospección ${x.pros} + Negociación ${x.neg}</div></div><div class="pts">${x.jury}</div><div class="bar"><div style="width:${width}%"></div></div></div>`
  }).join('');
}
function saveAndRender(){
  ...
  localStorage.setItem('dielca-score-mod60-brand-embedded',JSON.stringify(d));
...
function showChampion(){
  const data=getRankingData().sort((a,b)=>b.final-a.final)[0];
  document.getElementById('championTeam').textContent=data.final>0?data.name:'—';
  document.getElementById('championPoints').textContent=data.final+' pts';
  document.getElementById('champion').classList.add('active');
}
function hideChampion(){document.getElementById('champion').classList.remove('active')}
function resetAll(){
  if(confirm('¿Reiniciar todas las puntuaciones?')){
    localStorage.removeItem('dielca-score-mod60-brand-embedded');
    location.reload();
  }
}
